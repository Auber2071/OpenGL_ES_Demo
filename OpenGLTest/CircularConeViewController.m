...
#define kCount 101//顶点数量    必须为奇数
#define kVertexIndex   (kCount - 1) * 3
/**
...
@interface CircularConeViewController ()
...
@implementation CircularConeViewController {
...
- (void)viewDidLoad {
    ...
    self.effect.transform.projectionMatrix = GLKMatrix4MakePerspective(GLKMathDegreesToRadians(45), self.view.frame.size.width / self.view.frame.size.height, 0.1f, 10.0f);
}
 ...
 */
- (void)setupLighting{
    self.effect.light0.enabled = GL_FALSE;//是否开启光照
...
- (void)setupBufferVBOAndEBO {
    ...
    glBufferData(GL_ARRAY_BUFFER, sizeof(Vertex) * kCount, _circleVertex, GL_STATIC_DRAW);

    //设置_bufferIndexEBO
    glGenBuffers(1, &_bufferIndexEBO);//顶点索引缓冲对象：IBO/_bufferIndexEBO
    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, _bufferIndexEBO);
    glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(GLuint) * kVertexIndex, _vertextIndex, GL_STATIC_DRAW);
...
- (void)glkView:(GLKView *)view drawInRect:(CGRect)rect{
    ...
    glDrawElements(GL_LINE_LOOP, kCount, GL_UNSIGNED_INT, 0);
...
- (Vertex *)getBallCircleVertexWithNum:(GLint) num {
    ...
    GLfloat delta = (2 * M_PI) / (num-1); // 分割的份数
    GLfloat ballRaduis = 0.5; // 球的半径
    //顶点
    GLfloat pointZ = 0.0;
    GLfloat pointX = 0.0;
    GLfloat pointY = 0.0;

    Vertex * topVertex = malloc(sizeof(Vertex) * num);
    memset(topVertex, 0x00, sizeof(Vertex) * num);

    topVertex[0] = (Vertex){0, 0.5, 0.5};

    for (int i = 1; i < num; i++) {
        GLfloat angle = delta * i * M_PI;
        pointY = -0.5;
        pointX = ballRaduis * cos(angle);
        pointZ = ballRaduis * sin(angle);
        topVertex[i] = (Vertex){pointX, pointY, pointZ};
        NSLog(@"i: %d,angle:%f, pointX: %f, pointZ: %f",i ,angle, pointX, pointZ);
    }
    return topVertex;
}
...
- (GLuint *)getBallVertexIndexWithNum:(GLint)num {
    GLint count = (num - 1) * 3;
    GLint sizeNum = sizeof(GLuint) * count;
    GLuint * ballVertexIndex = malloc(sizeNum);
    memset(ballVertexIndex, 0x00, sizeNum);
    for (int i = 1; i < num; i++) {
        ballVertexIndex[i * 3] = 0;
        if (i == num - 1) {//最后一个
            ballVertexIndex[i * 3 + 1] = i;
            ballVertexIndex[i * 3 + 2] = 1;
        } else {
            ballVertexIndex[i * 3 + 1] = i;
            ballVertexIndex[i * 3 + 2] = i + 1;
        }
    }
